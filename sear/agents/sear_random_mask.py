...
class SEARRandomMaskAgent:
    ...
    def __init__(self, obs_shape, action_shape, device, lr, latent_dim,
                 ...
                 mask_loss_coeff, detach_critic, detach_decoders, num_patches,
                 patch_size):
        self.device = device
        ...
        self.fixed_target_mask = self.generate_random_mask()

    def generate_random_mask(self):
        patch_rows = np.random.randint(0,
                                       self.frame_resolution -
                                       (self.patch_size - 1),
                                       self.num_patches,
                                       dtype=int)
        patch_cols = np.random.randint(0,
                                       self.frame_resolution -
                                       (self.patch_size - 1),
                                       self.num_patches,
                                       dtype=int)
        mask = torch.zeros(
            (1, self.frame_stack, self.frame_resolution, self.frame_resolution))
        # TODO: Vectorize this more
        for d_row in range(self.patch_size):
            for d_col in range(self.patch_size):
                mask[:, :, patch_rows + d_row, patch_cols + d_col] = 1
    ...
    def update_critic_and_decoders(self, obs, encoded_obs, action, reward,
        ...
        reconstructed_obs = self.decoder(
            f1.detach() if self.detach_decoders else f1)
        reconstructed_mask = self.mask_decoder(
            f2.detach() if self.detach_decoders else f2)
        ...
        mask_loss = self.mask_loss_fn(
            reconstructed_mask,
            self.fixed_target_mask.expand(reconstructed_mask.shape))
        mask_loss = mask_loss.reshape(reconstructed_mask.shape[0],
                                      -1).sum(dim=1).mean()
    ...
    def get_frames_to_record(self, obs):
        ...
        masks = self.fixed_target_mask[0].detach().cpu().numpy() * 255.0
