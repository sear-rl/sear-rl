...
class FrameStackWrapper(dm_env.Environment):
    ...
    def __init__(self, env, num_frames, frame_keys=None):
        ...
        self._frames = [
            deque([], maxlen=num_frames) for _ in range(len(frame_keys))
        ]
        ...
        for key in frame_keys:
            ...
            wrapped_obs_spec[key] = specs.BoundedArray(shape=np.concatenate(
...
class ActionDTypeWrapper(dm_env.Environment):
    ...
    def __init__(self, env, dtype):
        ...
        self._action_spec = specs.BoundedArray(wrapped_action_spec.shape, dtype,
...
class SegmentationToRobotMaskWrapper(dm_env.Environment):
    ...
    def _transform_observation(self, time_step):
        ...
        robot_mask = np.logical_and(
            types == const.OBJ_GEOM,
            np.isin(ids, self._env.robot_segmentation_ids))
        robot_mask = robot_mask.astype(np.uint8)
        robot_mask = robot_mask.reshape(robot_mask.shape[0],
                                        robot_mask.shape[1], 1)
...
class SegmentationFilter(dm_env.Environment):
    ...
    def __init__(self, env, segmentation_key="segmentation"):
        ...
        if len(frame_shape) == 4:
            frame_shape = frame_shape[1:]
        new_shape = (int(frame_shape[0] / self.downsample),
                     int(frame_shape[1] / self.downsample), frame_shape[2])
        wrapped_obs_spec[segmentation_key] = specs.BoundedArray(
            shape=new_shape,
            dtype=np.uint8,
            minimum=0,
    ...
    def _transform_observation(self, time_step):
        obs = time_step.observation
        seg = obs[self.segmentation_key]
        filtered_seg = morphology.binary_opening(
            seg)[::self.downsample, ::self.downsample]
...
class NoisyMaskWrapper(dm_env.Environment):
    ...
    def _transform_observation(self, time_step):
        ...
        pixels_to_drop = np.random.binomial(1, self.prob_drop,
                                            mask.shape).astype(np.uint8)
...
class SlimMaskWrapper(dm_env.Environment):
    ...
    def _transform_observation(self, time_step):
        ...
        new_mask = rescale(mask, (1 / self.scale, 1 / self.scale, 1),
                           anti_aliasing=False)
        new_mask = rescale(new_mask, (self.scale, self.scale, 1),
                           anti_aliasing=False) * 255.0
...
class StackRGBAndMaskWrapper(dm_env.Environment):
    ...
    def __init__(self,
        ...
        wrapped_obs_spec = env.observation_spec()
        assert wrapped_obs_spec[
            segmentation_key].shape[:-1] == wrapped_obs_spec[rgb_key].shape[:-1]
        ...
        wrapped_obs_spec[new_key] = specs.BoundedArray(shape=new_shape,
                                                       dtype=np.uint8,
                                                       minimum=0,
                                                       maximum=255,
                                                       name="observation")
...
class Camera_Render_Wrapper:
    ...
    def render(self, height, width, *args, **kwargs):
        if self.camera is None or self.camera.height != height or self.camera.width != width:
            ...
            self.camera.set_pose(self.lookat, self.distance, self.azimuth,
                                 self.elevation)
...
class RandomCameraWrapper(dm_env.Environment):
    ...
    def __init__(self, env, lookats, distances, azimuths, elevations):
        ...
        self.render_wrappers = [
            Camera_Render_Wrapper(self._env.sim, lookat, distance, azimuth,
                                  elevation)
...
class CropWrapper(dm_env.Environment):
    ...
    def __init__(self, env, keys_to_crop, top_left, bottom_right):
        ...
        for key in keys_to_crop:
            ...
            new_spec = specs.BoundedArray(shape=tuple(frame_shape),
    ...
    def _transform_observation(self, time_step):
        ...
        for key in self.keys_to_crop:
            frame = obs[key][self.top_left[0]:self.bottom_right[0],
...
def get_env_observation_spec(env):
    return specs.BoundedArray(shape=env.observation_space.shape,
...
def get_env_action_spec(env):
    return specs.BoundedArray(shape=env.action_space.shape,
