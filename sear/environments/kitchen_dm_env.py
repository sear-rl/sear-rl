...
import sear.environments.kitchen_custom_envs  # Import to register kitchen custom environments to Gym
from sear.environments.wrappers import (
    ActionDTypeWrapper, ActionRepeatWrapper, Camera_Render_Wrapper,
    ExtendedTimeStepWrapper, FrameStackWrapper, NoisyMaskWrapper,
    RandomCameraWrapper, SegmentationFilter, SegmentationToRobotMaskWrapper,
    SlimMaskWrapper, StackRGBAndMaskWrapper, get_env_action_spec,
...
class Kitchen_Wrapper(dm_env.Environment):
    ...
    def __init__(self, env_name: str, discount=1.0, seed=None):
        ...
        self.physics = Camera_Render_Wrapper(self._env.sim,
                                             lookat=[-0.3, .5, 2.],
                                             distance=1.86,
                                             azimuth=90,
                                             elevation=-60)
...
def make_kitchen(name,
                 ...
                 use_rgbm=False,
                 slim_mask_cfg=None):
    assert camera_name in ["fixed", "random"]

    env = Kitchen_Wrapper(env_name=name, discount=discount, seed=seed)
    if camera_name == "random":
        env = RandomCameraWrapper(env,
                                  lookats=[[-0.3, .5, 2.]] * 6,
                                  distances=[1.86] * 6,
    ...
    if add_segmentation_to_obs:
        ...
        env = SegmentationToRobotMaskWrapper(env,
                                             segmentation_key,
                                             types_channel=1)
